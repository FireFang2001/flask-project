...
function updateFilterDateDisplay() {
    ...
    if (startDate) {
        ...
        // var dates = location.search.split('&')[1]+'&'+location.search.split('&')[2];
        // location.href = location.search.replace(dates, 'sd='+startDate+'&'+'ed='+endDate)
    } else {
...
$(document).ready(function(){
    ...
    var sork_key = queryData["sk"];
    $('#'+sork_key).addClass('active');
    $('#'+sork_key).siblings('li').removeClass('active');
    if (!sork_key) sork_key = "最新上线";
    if (sork_key == "new") sork_key = "最新上线";
    if (sork_key == "booking") sork_key = "入住最多";
    if (sork_key == "price-inc") sork_key = "价格 低-高";
    if (sork_key == "price-des") sork_key = "价格 高-低";
    $(".filter-title-bar>.filter-title").eq(2).children("span").eq(0).html(sork_key);
    ...
    $(".filter-item-bar>.filter-area").on("click", "li", function(e) {
        if (!$(this).hasClass("active")) {
            ...
            var aid = location.search.split('&')[0];
            var aname = location.search.split('&')[3];
            var newaname = $(this).html();

            var path = location.search.replace(aid, '?aid=' + new_aid);
            path = path.replace(aname, 'aname='+newaname);
            location.href = path;
...
$(document).ready(function(){
    ...
    $.get('/index/allsearch/'+path, function (data) {
        if (data.code == '200'){
        var house_detail = template('house-list', {houses:data.houses});
        $('.house-list').html(house_detail);

        var area_detail = template('area-list', {areas:data.areas});
        $('.filter-area').append(area_detail);
